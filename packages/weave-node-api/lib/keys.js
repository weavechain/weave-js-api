...
const encodeString = (plaintext, key, iv) => {
    ...
    const wkey = crypto.enc.Hex.parse(typeof key === "string" ? key : toHex(key))
    const wiv = crypto.enc.Hex.parse(typeof iv === "string" ? iv : toHex(iv));

    const encoded = crypto.AES.encrypt(plaintext.trim(), wkey, {
        iv: wiv,
        mode: crypto.mode.CBC,
        padding: crypto.pad.Pkcs7,
    }).toString();
...
const decodeString = (encoded, key, iv) => {
    ...
    let encrypted = crypto.lib.CipherParams.create({
        ciphertext: crypto.enc.Hex.parse(toHex(cipher)),
        formatter: crypto.format.OpenSSL
    });
    const decrypted = crypto.AES.decrypt(encrypted, wkey, {
        iv: wiv,
...
const encodeStringFor = (plaintext, fromPrivateKey, toPubKey, iv) => {
    ...
    const encoded = crypto.AES.encrypt(plaintext, wkey, {
        iv: wiv,
        mode: crypto.mode.CBC,
        padding: crypto.pad.Pkcs7,
    }).toString();
...
const decodeStringFrom = (encoded, fromPubKey, toPrivateKey, iv) => {
    const wkey = crypto.enc.Hex.parse(typeof fromPubKey === "string" ? fromPubKey : toHex(fromPubKey))
    const wiv = crypto.enc.Hex.parse(typeof iv === "string" ? iv : toHex(iv));
