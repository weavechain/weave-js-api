...
class ApiContext {
    ...
    initKeys(serverPublicKey, clientPublicKey, clientPrivateKey) {
        ...
        if (serverPublicKey.startsWith("weave")) {
            pub = ecc.keyFromPublic(ApiContext.deserializePublic(serverPublicKey), "bytes");
            ...
            pub = ecc.keyFromPublic(ApiContext.deserializePublic(this.serverPublicKey), "bytes");
        }
    ...
    static unpackKeys(privateKey) {
        ...
        if (bytes.length <= 33) {
            return ecc.keyFromPrivate(bytes.length === 33 ? bytes.subarray(1) : bytes);
        } else {
            return null;
        }
    }
    ...
    static deriveKeys(privateKey) {
        ...
        try {
            ...
            const key = ecc.keyFromPrivate(data.length === 33 ? data.subarray(1) : data);
            return ApiContext.encodeKey(key);
        } catch (e) {
            if (bytes.length <= 33) {
                const key = ecc.keyFromPrivate(bytes.length === 33 ? bytes.subarray(1) : bytes);
                return ApiContext.encodeKey(key);
            } else {
                return null;
            }
